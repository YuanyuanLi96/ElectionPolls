---
title: "Replicate_paper_result"
author: "Jiming Jiang, Yuanyuan Li, Peter X. K. Song"
output: 
  html_document:
...
This manual shows how to replicate the results in the paper. The codes are copied from "R/sme.R", while we present the results with codes in this mannual.
...
# Load all the results run by sme.R to speed up the Rmarkdown compile
...
```{r}
result1$eblup.fig#EBLUP based on Model 1
result1$mspe.fig#Prediction intervals based on Model 1
```
...
```{r}
result2$eblup.fig#EBLUP based on Model 2
result2$mspe.fig#Prediction intervals based on Model 2
```
...
```{r}
result3$eblup.fig#EBLUP based on Model 3
result3$mspe.fig#Prediction intervals based on Model 3
```
...
```{r}
result4$eblup.fig#EBLUP based on Model 4
result4$mspe.fig#Prediction intervals based on Model 4
```
...
```{r}
# Comparison of predictions: Model1, train on 2016, predict on 2020
model1_adjust1$wrong#States where at least one method yields wrong prediction
model1_adjust1$Actual_votes_sum#actual national support rates
model1_adjust1$SAE_votes_sum#national rate estimated by SAE
model1_adjust1$PoP_votes_sum#national rate estimated by PoP
```
...
write.table(model1_adjust2$wrong, sep=",",file="Results/Table2.csv",append = TRUE,row.names = FALSE)
```

```{r}
# Comparison of predictions: Model1, train on 2020, predict on 2016
...
write.table(model3_adjust1$wrong, sep=",",file="Results/Table2.csv",append = TRUE,row.names = FALSE)
```

```{r}
# Comparison of predictions: Model3, train on 2016, predict on 2020
...
write.table(model3_adjust2$wrong, sep=",",file="Results/Table2.csv",append = TRUE,row.names = FALSE)
```

```{r}
# Comparison of predictions: Model3, train on 2020, predict on 2016
...
## Ranking pollster using random effects
...
```{r}
t3_bottom#Ranking based on RE: Bottome-10
```
...
```
```{r}
t5#Ranking based on RE: Pollsters Participated in Both Elections; Top-10 
```
...
```
```{r}
t5_bottom#Ranking based on RE: Pollsters Participated in Both Elections;Bottom-10 
```

## Ranking pollster using mixed effects
```{r, eval=FALSE}
...
```{r}
t4_bottom#Ranking based on ME: Bottome-10
```
...
```
```{r}
t6#Ranking based on ME: Pollsters Participated in Both Elections; Top-10
```
...
```
```{r}
t6_bottom#Ranking based on ME: Pollsters Participated in Both Elections; Bottom-10
```
...
```{r}
model3_eblup_20$wrong#Comparison of Weighted Prediction Methods (Model 3)
```
...
write.table(model3_rank_20,file="Results/Table7.csv",sep =
              ",", row.names = F)
```
```{r}
model3_rank_20$wrong#Comparison of Weighted Prediction Methods (Model 3)
```
...
```{r}
model3_eblup_16$wrong#Comparison of Weighted Prediction Methods (Model 3)
```
...
#Compare raw diifference ditribution and log ratio distribution
ggplot(df, aes(x, group=type, col=type)) + geom_density(position='dodge')+
  scale_x_continuous(limits=c(-0.5,0.5),breaks = seq(-0.5, 0.5, 0.1),
                     minor_breaks = seq(-0.5,0.5,0.05))#log ratio is more normal
```
...
```
```{r}
model1$eblup_pp#EBLUP
model1$mspe_cycle#Prediction intervals
```
...
```{r}
model1.1$eblup_pp#EBLUP
model1.1$mspe_cycle#Prediction intervals
```
